...
const filePath = path.join(process.cwd(), 'dataforcurrencycondition.json');
...
const questionassertionandfillinganswer =async (task, page)=>{
   ...
   test('Requestcreation', async () => {
       ...
       let currencyAmountstr = currencyAmount.toString();
       let i = 3;
       for(let i =0;i<3;i++){
      ...
      //  await page.locator(`//label[@for='Currency question']/following-sibling::div//button`).click();
       // await page.locator("//div//span[text()='USD']").click();
       await page.waitForTimeout(3000);
       ...
       await logout(page);   
       currencyAmount = 1001;
       currencyAmountstr = currencyAmount.toString();
